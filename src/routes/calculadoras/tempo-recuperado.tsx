...
import { TimeBenefitChart, BikeComparison } from "@/components/mobility/TimeProjectionChart";
import {
  BikeResultCard,
  BudgetSelector,
  Metric,
        ...
        <section className="bg-ink text-ink-foreground">
          <div className="responsive-container py-6 sm:py-8">
            ...
            <h1 className="mt-2 max-w-4xl text-3xl font-black leading-tight sm:text-4xl">
              Quanto tempo a bike elétrica devolve por mês e por ano?
            </h1>
            ...
            <div className="rounded-lg bg-card p-5 ring-1 ring-line sm:p-6">
              ...
              <div className="mt-6 space-y-5">
                <div className="grid gap-4 sm:grid-cols-2">
                  <NumberField
                    name="currentMinutes"
                    label="Tempo atual por dia (ida + volta)"
                    value={currentMinutes}
                    onChange={setCurrentMinutes}
                    onBlur={() => mark("currentMinutes")}
                    suffix="min"
                    step="1"
                    help="Incluindo espera."
                    error={err("currentMinutes")}
                  />
                  <NumberField
                    name="bikeMinutes"
                    label="Tempo de bike por dia (ida + volta)"
                    value={bikeMinutes}
                    onChange={setBikeMinutes}
                    onBlur={() => mark("bikeMinutes")}
                    suffix="min"
                    step="1"
                    help="Sua estimativa para os mesmos trajetos."
            ...
            >
              ...
              {!time ? (
                ...
                <div className="mt-5 space-y-5">
                  <ResultHero
                    eyebrow="O tempo que pode mudar no seu ano"
                    value={hours(saved)}
                    favorable={saved > 0}
                    conclusion={
                      saved > 0
                        ? "a menos por ano no trajeto, considerando apenas os minutos que você informou. Isso não é uma promessa de tempo real."
                        : saved < 0
                          ? "a mais por ano de bike neste cenário. Avalie as outras razões para pedalar antes de decidir."
                          : "de diferença anual com os tempos informados."
                    }
                  />
                  <dl className="grid grid-cols-2 gap-3">
                    ...
                    <Metric
                      label={label("ano")}
                      value={`${hours(saved)} · ${decimal(Math.abs(time.savedFullDaysPerYear))} dias`}
                      emphasis
                    />
                  ...
                  <p
                    className={`rounded-md p-4 text-sm leading-relaxed ${saved > 0 ? "bg-mint/20 text-ink" : "bg-surface text-muted-foreground ring-1 ring-line"}`}
                  >
                    {insight}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Estimativa com {decimal(time.daysPerYear)} dias de trajeto por ano. "Dias" = blocos de 24 h.
                  </p>
            ...
            <section aria-labelledby="bikes" className="rounded-lg bg-card p-5 ring-1 ring-line sm:p-7">
              <div className="max-w-3xl">
                <h2 id="bikes" className="text-2xl font-black text-ink">
                  Bikes com autonomia para seu trajeto
                </h2>
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                  {QUICK_ORDER_CRITERION} Esta ferramenta não calcula economia financeira nem payback.
                </p>
              </div>
                  ...
                  <div className="mt-6 grid gap-5 lg:grid-cols-2">
                    {bikes.map((bike) => (
                      <BikeResultCard
                        ...
                        selected={false}
                        onSelect={() => {}}
                        position="calculadora_tempo_recuperado"
                      />
                    ))}
                  </div>
                  <div className="mt-5">
            ...
            <div className="mt-5 grid gap-6 text-sm leading-relaxed text-muted-foreground lg:grid-cols-2">
              <div>
                ...
                <ul className="mt-2 list-disc space-y-2 pl-5">
                  ...
                  <li>
                    Horas por mês = horas por ano ÷ 12 (equivale a semanas/ano ÷ 12 semanas por mês; com 52, 4,33).
                  </li>
              ...
              <div>
                ...
                <p className="mt-2">
                  ...
                  autonomia declarada com margem de {Math.round((AUTONOMY_SAFETY_MARGIN - 1) * 100)}%. Garupa e
                  orçamento são filtros rígidos. Nenhum valor financeiro é calculado aqui. Nenhum nome, e-mail ou
                  telefone é coletado.
                </p>
              </div>
            </div>
