...
import {
  computeTransportePublicoVsBike,
  DOMINANT_LABEL,
  SUBSET_PERCENT,
  transportePublicoInsight,
} from "@/lib/mobility/transporte-publico-vs-bike";
        ...
        <section className="bg-ink text-ink-foreground">
          <div className="responsive-container py-6 sm:py-8">
            ...
            <p className="mt-3 max-w-3xl text-sm leading-relaxed text-ink-foreground/80 sm:text-base">
              Informe só as viagens que pensa em trocar: gasto, tempo e distância. Dinheiro e tempo aparecem na hora,
              com até duas bikes reais, sem cadastro.
            </p>
          </div>
        </section>
            ...
            <div className="rounded-lg bg-card p-5 ring-1 ring-line sm:p-6">
              ...
              <div className="mt-6 space-y-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <NumberField
                    name="dailyKm"
                    label="Km por dia nessas viagens"
                    help="Distância total atual dessas viagens por dia (ida + volta), antes da troca."
                    value={dailyKm}
                    onChange={setDailyKm}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <NumberField
                    name="currentMinutes"
                    label="Tempo por dia hoje nessas viagens"
                    value={currentMinutes}
                    onChange={setCurrentMinutes}
                    onBlur={() => markTouched("currentMinutes")}
                    suffix="min"
                    step="1"
                    help="Ida + volta, incluindo espera."
                    error={visibleError("currentMinutes")}
                  />
                  <NumberField
                    name="bikeMinutes"
                    label="Tempo estimado de bike por dia"
                    value={bikeMinutes}
                    onChange={setBikeMinutes}
                    onBlur={() => markTouched("bikeMinutes")}
                    suffix="min"
                    step="1"
                    help="Sua estimativa para as mesmas viagens, ida + volta."
                    error={visibleError("bikeMinutes")}
                  />
                </div>
            ...
            >
              ...
              {!data ? (
                ...
                <div className="mt-5 space-y-5">
                  <ResultHero
                    eyebrow="Ônibus ou bike no seu trajeto"
                    value={brl(Math.abs(data.monthlySavings), true)}
                    favorable={data.monthlySavings > 0}
                    conclusion={
                      data.monthlySavings > 0
                        ? "de economia mensal estimada nos trajetos substituídos. Veja também o efeito no tempo e o investimento por modelo abaixo."
                        : data.monthlySavings < 0
                          ? "de custo mensal adicional estimado com a bike. O tempo pode mudar essa decisão; confira o resultado completo."
                          : "de diferença mensal com os dados informados. Confira também o tempo de trajeto."
                    }
                  />
                  <dl className="grid grid-cols-2 gap-3">
                    ...
                    <Metric
                      label={
                        time && time.savedHoursPerYear < 0 ? "Tempo adicional por ano" : "Tempo recuperado por ano"
                      }
                      value={hoursLabel}
                    />
                    <Metric
                      label="Benefício dominante"
                      value={costResult?.ok ? DOMINANT_LABEL[costResult.dominant] : "—"}
                    />
                  </dl>
                  <p
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    Estimativa, não garantia. Custo operacional da bike {brl(data.bikeTotalCost, true)}/mês; o preço
                    real entra no gráfico, no payback e nos cards.
                  </p>
            ...
            <section aria-labelledby="bikes" className="rounded-lg bg-card p-5 ring-1 ring-line sm:p-7">
              ...
              {!hasBikeUse ? (
                <p className="mt-5 rounded-md bg-surface p-4 text-sm text-muted-foreground ring-1 ring-line">
                  Sem uso de bike para comparar, nenhum modelo é sugerido.
                </p>
              ) : !sourceOk ? (
                <p className="mt-5 rounded-md bg-surface p-4 text-sm text-muted-foreground ring-1 ring-line">
                  ...
                  <div className="mt-6 grid gap-5 lg:grid-cols-2">
                    {paybacks.map(({ bike, projection }) => (
                      <BikeResultCard
                        ...
                        position="calculadora_transporte_publico_vs_bike"
                      />
                    ))}
                  </div>
                  <div className="mt-5">
            ...
            <div className="mt-5 grid gap-6 text-sm leading-relaxed text-muted-foreground lg:grid-cols-2">
              <div>
                ...
                <ul className="mt-2 list-disc space-y-2 pl-5">
                  <li>
                    Os campos descrevem só as viagens que você escolheu trocar, então usamos 100% desse conjunto —
                    nenhum percentual é presumido.
                  </li>
                  <li>
                    Tempo por ano = (minutos atuais − minutos de bike) × dias/semana × 52 semanas ÷ 60. Negativo = a
                    bike demora mais.
                  </li>
                  <li>
                    Benefício dominante: "dinheiro" se a economia mensal for positiva, "tempo" se as horas recuperadas
                    forem positivas, ambos ou nenhum.
                  </li>
                  <li>Km de bike por mês = km/dia × dias/semana × {decimal(WEEKS_PER_MONTH, 4)} (52 ÷ 12).</li>
                  <li>
                    Custo da bike = km substituídos × {brl(QUICK_BIKE_COST.energyPerKm, true)}/km +{" "}
              ...
              <div>
                ...
                <ul className="mt-2 list-disc space-y-2 pl-5">
                  ...
                  <li>
                    Tempos são os que você informou; não usamos mapas nem velocidade presumida. Reajuste de tarifa e
                    integração não são modelados.
                  </li>
                </ul>
              </div>
              <div>
