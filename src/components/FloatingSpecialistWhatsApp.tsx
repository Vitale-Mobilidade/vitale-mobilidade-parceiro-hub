...
const HIDE_KEY = "vitale_hide_floating_whatsapp";
...
export function FloatingSpecialistWhatsApp({
  ...
  const [hidden, setHidden] = useState<boolean>(() => {
    ...
    try {
      return sessionStorage.getItem(HIDE_KEY) === "true";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem(HIDE_KEY) === "true") setHidden(true);
    } catch {}
  }, []);
    ...
    >
      ...
      <div className="relative flex items-end gap-2">
        <button
          ...
          className="hidden sm:flex flex-col items-start text-left bg-white rounded-2xl shadow-lg px-4 py-2.5 hover:shadow-xl transition-shadow border border-black/5 max-w-[220px]"
          style={{ color: "#0F172A" }}
        ...
        <button
          ...
          onClick={handleClick}
          data-event="floating_specialist_whatsapp_clicked"
          className="relative flex items-center justify-center rounded-full shadow-lg hover:shadow-xl transition-all active:scale-95"
        ...
        >
          ...
          <span
            ...
            onClick={handleClose}
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") handleClose(e as any);
            }}
            aria-label="Fechar"
            className="absolute -top-1.5 -right-1.5 flex items-center justify-center bg-white rounded-full shadow border border-black/10 hover:bg-gray-50"
      ...
      <div className="sm:hidden mt-1 text-right">
        <button
          ...
          onClick={handleClick}
          data-event="floating_specialist_whatsapp_clicked"
          className="bg-white rounded-xl shadow px-2.5 py-1 text-[11px] font-semibold border border-black/5"
          style={{ color: "#0F172A" }}
        >
          Ainda em dúvida? Fale com o especialista
        </button>
      </div>
    </div>
  );
