        ...
        <>
          ...
          <div style={fabStyle}>
            <button
              ...
              className={`flex items-center gap-2 pl-3 pr-4 h-14 rounded-full shadow-lg hover:shadow-xl transition-all active:scale-95 bg-primary text-primary-foreground font-semibold${compactHome ? " max-sm:w-14 max-sm:justify-center max-sm:p-0" : compactOnMobile ? " max-md:w-14 max-md:justify-center max-md:p-0" : ""}`}
            >
              <span className="h-9 w-9 rounded-full bg-white/25 flex items-center justify-center">
                <MessagesSquare className="h-5 w-5" />
              </span>
              <span className={`text-[14px] leading-tight text-left${compactHome ? " max-sm:hidden" : compactOnMobile ? " max-md:hidden" : ""}`}>
