...
export function VehicleVsBikeCalculator({
  sourceOk,
  candidates,
  copy,
}: {
  ...
  const errors = useMemo(
    () => ({
      ...
      fixedAvoided:
        keepsVehicle === false && fixedAvoided.trim() !== ""
  ...
  const insight = data
    ? vehicleVsBikeInsight({
        vehicle: copy.vehicle,
        replaceablePercent: values.monthlySpend > 0 ? 100 : 0,
        monthlySavings: data.monthlySavings,
        annualSavings: data.annualSavings,
        keepsVehicle: keepsVehicle === true,
        fixedIncluded: data.currentFixedRemoved,
      })
    : null;
  const paybackLabel =
    !selectedPayback || !selectedPayback.projection.ok
      ? "—"
      ...
      <main>
        <section className="bg-ink text-ink-foreground">
          <div className="responsive-container py-6 sm:py-8">
            ...
            <h1 className="mt-2 max-w-4xl text-3xl font-black leading-tight sm:text-4xl">{copy.h1}</h1>
            <p className="mt-3 max-w-3xl text-sm leading-relaxed text-ink-foreground/80 sm:text-base">{copy.intro}</p>
          </div>
        </section>
            ...
            <div className="rounded-lg bg-card p-5 ring-1 ring-line sm:p-6">
              ...
              <div className="mt-6 space-y-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <NumberField
                    name="daysPerWeek"
                    label="Dias por semana desses trajetos"
                    value={daysPerWeek}
                ...
                <fieldset>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    {(
                      ...
                      <label
                        key={label}
                        className={`flex min-h-11 cursor-pointer items-center justify-center rounded-md px-3 text-sm font-bold ring-1 focus-within:ring-2 focus-within:ring-action ${keepsVehicle === v ? "bg-action text-action-foreground ring-action" : "bg-card text-ink ring-line"}`}
                      >
                        <input
                          type="radio"
                          name="keepsVehicle"
                          className="sr-only"
                          checked={keepsVehicle === v}
                          onChange={() => setKeepsVehicle(v)}
                        />
                        {label}
                      </label>
                    ))}
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    Responder "Não" não adiciona nenhum valor sozinho. Seguro, IPVA e outros custos fixos só entram se
                    você responder "Não" e digitar o valor em "Ajustar premissas".
                  </p>
                  {keepsVehicle === false && (
                    <details className="mt-3 rounded-md bg-surface p-3 ring-1 ring-line">
                      <summary className="min-h-11 cursor-pointer text-sm font-bold text-ink">
                        Ajustar premissas (opcional)
                      </summary>
                      <div className="mt-3">
                        <NumberField
                          name="fixedAvoided"
                          label="Custo fixo mensal que deixará de existir"
                          value={fixedAvoided}
                          onChange={setFixedAvoided}
                          onBlur={() => markTouched("fixedAvoided")}
                          suffix="R$/mês"
                          help={`Seguro, IPVA, licenciamento etc. que de fato desaparecem sem ${copy.withArticle}. Em branco = nada somado.`}
                          error={visibleError("fixedAvoided")}
                        />
                      </div>
                    </details>
                  )}
            ...
            >
              ...
              {!data ? (
                ...
                <div className="mt-5 space-y-5">
                  <ResultHero
                    eyebrow={`${copy.vehicle} ou bike no seu trajeto`}
                    value={brl(Math.abs(data.monthlySavings), true)}
                    favorable={data.monthlySavings > 0}
                    conclusion={
                      data.monthlySavings > 0
                        ? `de economia mensal estimada ao substituir o uso ${copy.ofArticle} informado. O preço da bike entra na projeção abaixo.`
                        : data.monthlySavings < 0
                          ? "de custo mensal adicional estimado com a bike neste cenário. Compare o custo total antes de decidir."
                          : "de diferença mensal com os dados informados. O investimento inicial é analisado por bike."
                    }
                  />
                  <dl className="grid grid-cols-2 gap-3">
                    <Metric
                      label={`Gasto evitável ${copy.ofArticle}/mês`}
                      value={brl(data.currentTotalReplaced, true)}
                    />
                    <Metric label="Custo operacional da bike/mês" value={brl(data.bikeTotalCost, true)} />
                    <Metric
                      label={data.monthlySavings >= 0 ? "Economia líquida mensal" : "Diferença líquida mensal"}
                      value={brl(data.monthlySavings, true)}
                      emphasis
                    />
                    <Metric
                      label={data.annualSavings >= 0 ? "Economia líquida anual" : "Diferença líquida anual"}
                      value={brl(data.annualSavings, true)}
                      emphasis
                    />
                    <Metric
                      label={selectedBike ? `Payback estimado · ${selectedBike.name}` : "Payback estimado"}
                      value={paybackLabel}
                    />
                  </dl>
                  <p
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    Estimativa, não garantia. Economia sem o preço da bike; o preço real entra no payback, no gráfico e
                    nos cards.
                  </p>
            ...
            <section aria-labelledby="bikes" className="rounded-lg bg-card p-5 ring-1 ring-line sm:p-7">
              ...
              {!hasBikeUse ? (
                <p className="mt-5 rounded-md bg-surface p-4 text-sm text-muted-foreground ring-1 ring-line">
                  Com 0% de substituição não existe uso de bike para comparar, então não sugerimos nenhum modelo.
                </p>
              ) : !sourceOk ? (
                <p className="mt-5 rounded-md bg-surface p-4 text-sm text-muted-foreground ring-1 ring-line">
            ...
            <div className="mt-5 grid gap-6 text-sm leading-relaxed text-muted-foreground lg:grid-cols-2">
              <div>
                ...
                <ul className="mt-2 list-disc space-y-2 pl-5">
                  <li>
                    Você informa só o gasto variável, km e dias dos trajetos que faria de bike; esse conjunto inteiro é
                    considerado substituído.
                  </li>
                  <li>
                    Gasto evitável = gasto variável informado (+ custo fixo evitado, somente se você disser que não
                    continuará com {copy.withArticle} e digitar o valor).
                  </li>
