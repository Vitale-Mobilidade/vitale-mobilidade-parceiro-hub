...
export function CostProjectionChart({
  ...
  const gap =
    ...
    [...chartPoints]
      .reverse()
      .map((point, reversedIndex) => `${x(chartPoints.length - reversedIndex - 1)},${y(point.bikeCostWithPurchase)}`)
      .join(" ");

  return (
    <figure
      aria-labelledby="projection-title projection-caption"
      className="overflow-hidden rounded-lg bg-surface p-4 ring-1 ring-line"
    >
      <figcaption id="projection-title" className="font-bold text-ink">
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-xs text-muted-foreground" aria-hidden="true">
        <span className="flex items-center gap-2">
          <span className="h-0.5 w-6 bg-ink" /> Trajeto atual
        </span>
        <span className="flex items-center gap-2">
          <span className="h-0.5 w-6 bg-action" /> Bike + operação
        </span>
      </div>
      <svg
        ...
        role="img"
        aria-label={`Comparação de custos acumulados em 12, 24 e 36 meses para ${bikeName}; a área colorida mostra a distância entre os custos`}
      >
        {[0, 0.5, 1].map((ratio) => {
          ...
          return (
            <g key={ratio}>
              ...
              <text x="2" y={lineY + 4} className="fill-muted-foreground text-[11px]">
        ...
        {chartPoints.map((point, index) => (
          <g key={point.months}>
            ...
            <text x={x(index)} y={HEIGHT - 5} textAnchor="middle" className="fill-muted-foreground text-[11px]">
              {point.months}m
            </text>
          </g>
        ))}
