...
- Lista `/radar`: seção "Histórico arquivado (N)" no fim do catálogo, fora dos rankings, do destaque e dos indicadores ("Bikes monitoradas" conta só ofertas ativas). Cada item é um link para o detalhe, com marcador vermelho + "Sem oferta no Mercado Livre", `Último preço registrado: [valor]` e `Registrado em [data] · pode não ser o preço de hoje`. Sem farol, sem CTA do Mercado Livre e sem alerta de preço.
- Detalhe `/radar/{bikeId}` sem oferta atual: selo neutro "Histórico arquivado", bloco "Último preço registrado" com o valor real e a data (nunca zerado, nunca fictício), e painel "Histórico registrado" (gráfico `compact`) com contagem real de dias confirmados/reconstruídos e data inicial. Nunca há preço atual, farol, CTA de compra ou alerta.
