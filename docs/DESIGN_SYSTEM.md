...
- **Regra do histórico curto/descontínuo (`classification === "forming"`)**: leitura **descritiva**, nunca prescritiva. Não há selo no cabeçalho do painel nem no topo do detalhe (`PriceStatus` só aparece com leitura conclusiva) — histórico curto não vira badge dominante. O texto diz "Comparação baseada nos registros disponíveis até agora para esta bike", sem "bom preço", "barato", "caro" ou percentual de desconto. A escala verde/amarelo/vermelho continua colorida como referência visual, com marcador neutro (`bg-ink`) na posição do preço atual e rótulos "Menor registrado / Mediana dos registros / Maior registrado". Com um único preço (`distinctPrices < 2`) nenhuma faixa ou posição é renderizada. Os registros existentes são a base operacional validada manualmente pelo time: a cobertura mede continuidade do monitoramento, não a validade dos preços.
