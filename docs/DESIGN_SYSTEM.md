...
- Alertas: modal coleta nome, WhatsApp, e-mail e meta de preço com consentimento; a confirmação atesta somente o cadastro salvo. A interface não expõe o estado operacional dos disparos nem afirma que um aviso já foi enviado. O backend permanece responsável pela persistência e pela ativação futura dos envios.
...
- Um único painel contínuo `PriceIntelPanel` (`src/components/radar/PriceIntelPanel.tsx`) substitui as duas caixas ("O preço atual está bom?" + "Resumo do período") e a seção separada de histórico. Estrutura em três faixas separadas por `border-line`, sem card dentro de card:
  ...
  2. régua farol segmentada, pin com preço atual e referências de menor/maior e faixa habitual quando calculável, sem texto operacional de cobertura;
  3. seletor público 7/14/30 dias e gráfico compacto com curva monotônica (interpolação apenas visual entre fechamentos registrados). As janelas 90/Tudo permanecem somente para cálculos internos; a explicação operacional não é exibida ao público.
...
- **Sem selos de avaliação de preço (regra final do responsável)**: o detalhe do Radar não exibe selo/badge de avaliação nem texto operacional de cobertura. O painel mostra apenas preço atual, extremos registrados e, quando calculável, a faixa habitual. Com histórico curto/descontínuo (`classification === "forming"`), o rótulo central é "Valor central". Com um único preço (`distinctPrices < 2`), não há faixa nem posição comparativa. A continuidade do monitoramento permanece um dado interno, não um parecer público sobre a validade dos preços.
- A classificação `lowest`/`good`/`typical`/`above` já calculada por `dailyMetrics` pode compor a faixa visual quando há dados suficientes; o painel não recalcula nem cria preços.
...
- `PriceIntelPanel` informa a quantidade de registros disponíveis na janela e a data da última verificação. Em `forming`, uma linha adicional informa que ainda não há sequência suficiente para qualificar o preço. A regra de `dailyMetrics` não mudou.
