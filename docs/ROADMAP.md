...
- **Etapa 3**: não concluída. Otimização estática de bundle no rascunho: `RadarAssistant` passou a ser carregado por `React.lazy` + `Suspense` em `src/routes/__root.tsx`, mantendo a exclusão de `/escolherbike` e `/painel-bikes`; sem medida real de PageSpeed/CWV e sem validação no preview.
