...
| 9 | Radar `/radar` | Parcial — **cutover no preview, aguarda publicação** | `/radar` e `/radar/$bikeId` (loader/head/503/404 em `src/lib/radar-routes.tsx`); `/acompanhamento[/{bikeId}]` → **301 real** em `src/server.ts` (ID literal e query preservados; comprovado por HTTP local); menu, rodapé, Home, `/bikes`, assistente e cards apontam para `/radar`; sitemap com `/radar` e `/bikes`, sem `/acompanhamento`. Mesmos links `meli.la` e preços antes/depois | Publicar e conferir 301 no domínio; detalhes `/radar/{id}` e `/bikes/{slug}` no sitemap só com geração dinâmica confiável. Rollback: remover bloco de redirect em `src/server.ts` + reverter links; sem DB |
