...
> **Direção aprovada em 25/09/2026 (pré-publicação):** Etapas 2 (SEO/GEO) e 3 (validação de SSR/performance) ficam deliberadamente adiadas, sem alteração de seus percentuais. A execução atual retoma a Etapa 4 (taxonomia) e a Etapa 5 (Home B2C), preserva as Etapas 6–8 já operacionais e prepara a continuidade da Etapa 9. O incremento local conecta Home/header/footer/Quiz ao ecossistema, move a comparação de `/bikes` para o estado de `/radar` e mantém `/bikes*` acessível como legado de transição. Publicação depende do gate registrado em `docs/STAGE4_HOME_2026-09-25.md`.
