...
Estado: publicado em 25/09/2026 após autorização do responsável. `pnpm validate` passou e a página pública foi inspecionada no navegador interno. A revisão não identificou mudança na origem do preço, oferta, histórico, Quiz ou clique afiliado.

Revisão multidisciplinar após o release: Produto **Pass** (hierarquia segue a decisão do responsável); Arquitetura **Pass** (componentes e RPC existentes); IA **N/A** (sem geração); Segurança **Pass** (sem entrada, segredo ou permissão nova); UX/UI **Pass** (pin rotulado, ordem mobile e desktop, banners compartilhados; página pública inspecionada); CX **Pass** (sem operação nova); Growth **Pass** (oferta direta preservada); PMO/QA **Pass** (`pnpm validate` e árvore remota idêntica ao código validado). O resultado em ranking e performance real ainda não foi medido.
...
| 20    | Newsletter                     | Captação local preparada, ainda não publicada                                                                                                                                | Formulário Home com nome/e-mail/consentimento, função de registro e migration aditiva na branch de UX do Radar                                                                                                                                                                                                                                                                           | Publicação coordenada de DB/função/web, verificação de RLS e consentimento; provedor e envio continuam pendentes                                                |
...
- `/calculadoras/tempo-recuperado` (23/09/2026, PREVIEW, não publicado): reusa `computeMobilityTime`, `computeTimeProjection` e `TimeProjectionChart`; métricas: horas recuperadas (ou tempo adicional) por mês/ano/3/5 anos; até 2 bikes reais só com km/dia, sem payback/economia. Analytics `calculadora_tempo_recuperado`; sitemap e card. Com isso as 9 rotas existem em PREVIEW. Correções: Tempo no trânsito agora pede ida e volta separadas (somadas no adaptador); `BikeResultCard` sem projeção usa elemento não interativo em vez de botão inerte.
...
## Revisão de UX/CRO das ferramentas — piloto Tempo no Trânsito (23/09/2026)

- A nova experiência de `/calculadoras/tempo-no-transito` foi implementada no mesmo projeto Lovable/TanStack Start: conclusão dominante em horas recuperadas, projeção comparativa de 1/3/5 anos, até duas bikes reais com contexto do trajeto, oferta direta, comparação, Radar, Quiz visual, vídeos reais quando disponíveis e próximas ferramentas. Não há números comerciais inventados.
...
## Ferramentas de Mobilidade — sete oficiais (26/09/2026, PREVIEW)
...
- [ ] Validação externa e publicação (não publicado).
