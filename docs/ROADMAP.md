...
| 9 | Radar `/radar` | Parcial — **rotas-alvo em preview, aguardam cutover** | `/radar` e `/radar/$bikeId` reutilizam loader/head/503/404 de `src/lib/radar-routes.tsx` e as mesmas páginas de `/acompanhamento`; links internos (cards, busca, breadcrumb, assistente) seguem a base via `src/lib/radar-base.ts`. HTML comparado: mesmos links `meli.la` e preços; `/radar/ID-inválido` = 404 | Publicar; decidir redirect 301 `/acompanhamento`→`/radar`, canonical único, nav global/Home/sitemap (ainda apontam ao legado) |
