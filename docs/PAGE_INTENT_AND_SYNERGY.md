...
> **Decisão vigente — 25/09/2026 (pré-publicação):** o header global contém `Radar`, `Conteúdos`, `Ferramentas` e CTA `Escolher minha bike`. O footer mantém esses hubs, Quiz e Grupo de Ofertas em hierarquia secundária. A Home orienta o visitante entre Quiz, Radar, Ferramentas, Conteúdos e comparação dentro do Radar. A exceção antiga que removia header/footer do Quiz foi revogada pelo responsável; a lógica, elegibilidade e CTA afiliado do resultado continuam intactos. `/bikes*` segue como transição legada, sem remoção ou redirect nesta entrega.
...
| `/ferramentas`                                      | "Que ferramenta me ajuda a decidir?"         | Orientação             | Busca por ferramenta, menu     | Lista das ferramentas que funcionam hoje          | Quiz (`/escolherbike`)                                              | Radar (`/radar`); catálogo (`/bikes`)                                           | Sem dados próprios: apenas rotas funcionais                                                           | Publicada           |
| `/comparar`                                         | "A ou B?"                                    | Avaliação              | Busca comparativa              | —                                                 | — (não citada em `/ferramentas`: sem promessa pública)              | —                                                                               | Sem contrato de specs                                                                                 | Futura              |
| `/calculadoras/*` (9 rotas)                         | "Quanto economizo?" / "Compensa?"            | Avaliação              | Busca por ferramenta           | Resultado imediato na própria tela                | Bikes reais compatíveis (máx. 2) e Quiz contextual                  | Radar (quando a bike é monitorada); `/ferramentas`                              | Sem dados próprios: cálculo determinístico + catálogo Supabase                                        | Publicadas          |
...
- Ferramentas: `/ferramentas` é rota real **publicada** (deployment `7faa0513-44fa-4055-99c2-6b2966e6417d`; 200 com H1, canonical, seção "Um caminho para decidir" e links reais verificados no HTML SSR). `/calc` e `/calc/` respondem **301** para `/ferramentas` preservando query/UTM (verificado no domínio). Na Home, o bloco da calculadora tem ID canônico `#ferramentas` e mantém `#calc` como âncora alias para backlinks antigos.
...
## `/calculadoras/economia` e `/calculadoras/payback` — calculadoras rápidas (snapshot pré-publicação de 23/09/2026)

> **Estado atual (23/09/2026):** este brief é um snapshot da época do preview. As 9 rotas `/calculadoras/*` já foram **publicadas** (ver `docs/ROADMAP.md`) e são listadas em `/ferramentas`. `/comparar` e `/conteudos` continuam futuros e proibidos como links. O Quiz `/escolherbike` segue terminal, focado no clique direto ao Mercado Livre.
